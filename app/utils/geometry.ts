...
/**
 * Maps backend geometry response (GeoJSON [lon, lat], Polyline string, or Gateway object) 
...
export function mapGeometryToLeafletPoints(data: any): LeafletPoint[] {
  ...
  // Case 2: Object with 'trace' polyline string (API Gateway response)
  if (typeof data.trace === 'string') {
    try {
      return polyline.decode(data.trace) as LeafletPoint[]
    } catch {
      // Fallback
    }
  }

  // Case 3: GeoJSON style object { coordinates: [[lon, lat], ...] }
  ...
  // Case 4: GeoJSON geometry property { geometry: { coordinates: ... } }
  ...
  // Case 5: OTP itineraries structure
  const legs = data?.data?.plan?.itineraries?.[0]?.legs || data?.plan?.itineraries?.[0]?.legs
