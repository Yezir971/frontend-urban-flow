<template>
  <Teleport to="body">
    ...
    >
      ...
      <!-- Conteneur Modal -->
      <div
        class="relative w-full max-w-xl bg-white rounded-[32px] p-6 sm:p-8 shadow-2xl overflow-hidden transform transition-all flex flex-col gap-6 z-10 my-auto border border-gray-100 max-h-[85vh]"
      >
        <!-- En-tête -->
        <div class="flex items-center justify-between">
          ...
          <button
            type="button"
            @click="closeModal"
            class="w-9 h-9 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-full flex items-center justify-center transition-all cursor-pointer"
          >
            <X class="w-5 h-5" />
          </button>
        </div>
        ...
        <div class="flex flex-col gap-3 overflow-y-auto pr-1">
          <div
            v-if="trips.length === 0"
            class="py-12 text-center text-gray-500 flex flex-col items-center justify-center gap-2"
          >
          ...
          <div
            ...
            class="flex items-center justify-between p-3.5 sm:p-4 rounded-2xl bg-[#F8FAF9] hover:bg-[#F0F5F2] border border-gray-100 hover:border-emerald-200 cursor-pointer transition-all duration-150 group"
          >
            <div class="flex items-center gap-3 min-w-0">
              <!-- Icône du mode dans un cercle vert foncé -->
              <div class="w-10 h-10 rounded-full bg-[#104E35] flex items-center justify-center text-white shrink-0 group-hover:scale-105 transition-transform">
            ...
            <div class="flex items-center gap-2 shrink-0 ml-2">
              <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-[#C6F0DB] text-xs font-bold text-[#104E35]">
                <Leaf class="w-3 h-3" />
                {{ formatCo2Value(trip.co2_saved_kg) }} saved
              </span>
              <ChevronRight class="w-4 h-4 text-gray-400 group-hover:text-[#104E35] transition-colors" />
            </div>
          </div>
        </div>
      </div>
